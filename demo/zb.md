...
在没有大数据概念之前，很多时候是用数据库去做一些显性关联分析，而当有了大数据概念之后，更多地其实是想做隐性的关联分析与挖掘，也就是结合不同类型的数据，然后找到其中的联系。因此这其中的趋势，实际就是目前的数据关系挖掘更加偏向于跨领域数据或者跨类型数据的综合分析。
...
- [](https://www.cnblogs.com/loser1949/p/8326047.html)
